...
/*
if(!(PIND & (1 << 2))) //botao de incremento
{
	_delay_ms(10);
	novo++;
}
if(!(PIND & (1 << 3))) //botao de decremento
{
	_delay_ms(10);
	novo--;
}
if(!(PIND & (1 << 4))) //botao de set
{
	_delay_ms(10);
	danger = novo;
	c++;			  //variavel de controle se ha novo set
}
...
resultadoADC = ADCW; // obtem o valor convertido (ADCH:ADCL)
temp = resultadoADC*5*100L/1024; //Define o tempo em alto de acordo com a entrada analogica
//o L serve para converter diretamente o valor binario
...
lcd_write_string_4d(" graus"); //funcao de escrita
_delay_ms(1000);
lcd_write_instruction_4d(lcd_Clear);//funcao de "limpar" o display
_delay_ms(4);
}
*/